...
export class GenericRoom extends Room<Schema | BaseState> {
  ...
  async onCreate(options: DynamicRoomOptions) {
    const { definitionSlug } = options || {};

    this.setMetadata({ definitionSlug });
    console.log(
      `[GenericRoom] Creating room for definition: ${definitionSlug}`
    );

    try {
      // Load game definition from DSL directory
      console.log(`[GenericRoom] 🔍 Loading definition for: ${definitionSlug}`);
      console.log(`[GenericRoom] 🔍 Options:`, options);

      this.definition = await loadDefinition({
        definitionId: definitionSlug || "multistep-quiz",
      });
